...
The research question is whether conditional variance models improve
one-session-ahead variance or VaR forecasts over transparent rolling
historical benchmarks on identical historical target dates.

All model inputs are decimal simple returns from the public
`historical-asset-risk-engine` v0.1.0 `simple_returns.csv` contract. Research
series are the configured upstream instruments plus an optional configured
...
The variance benchmark uses exactly 252 trailing returns, including
the origin return, and calculates sample variance with `ddof=1`. It rejects
non-finite or non-positive variance rather than clipping it. The VaR
...
retry using the protocol-defined starting values. Fits must have positive omega,
...
reported by series and across all configured series; lower values are better.
...
Final-test descriptive breakdowns are generated for every calendar year
covered by the configured test period.
...
The implementation and protocol configuration define the authoritative model
equations, fitting policy, diagnostics, and scoring rules.
